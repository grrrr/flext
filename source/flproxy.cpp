...
// === proxy class for Flext ============================

#if FLEXT_SYS == FLEXT_SYS_PD

t_class *Flext::px_class = NULL;

void Flext::px_object::px_method(px_object *obj,const t_symbol *s,int argc,t_atom *argv)
{
    obj->base->m_methodmain(obj->index,s,argc,argv);
}

void Flext::cb_px_anything(t_class *c,const t_symbol *s,int argc,t_atom *argv)
{
    thisObject(c)->m_methodmain(0,s,argc,argv);
}

#define DEF_IN_FT(IX) \
void Flext::cb_px_ft ## IX(t_class *c,float v) { \
...
void Flext::cb_px_anything(t_class *c,const t_symbol *s,short argc,t_atom *argv)
{
    // check if inlet allows anything (or list)
    
    Flext *o = thisObject(c);
    int ci = ((FlextHdr *)o->x_obj)->curinlet;
...
void Flext::cb_px_int(t_class *c,long v)
{
    Flext *o = thisObject(c);
...
void Flext::cb_px_float(t_class *c,double v)
{
    Flext *o = thisObject(c);
    int ci = proxy_getinlet((t_object *)o->x_obj);
...
void Flext::cb_px_bang(t_class *c)
{
...
#define DEF_IN_FT(IX) \
void Flext::cb_px_in ## IX(t_class *c,long v) { long &ci = ((FlextHdr *)thisObject(c)->x_obj)->curinlet; ci = IX; cb_px_int(c,v); ci = 0; } \
void Flext::cb_px_ft ## IX(t_class *c,double v) { long &ci = ((FlextHdr *)thisObject(c)->x_obj)->curinlet; ci = IX; cb_px_float(c,v); ci = 0; }
...
void Flext::SetProxies(t_class *c)
{
    // proxy for extra inlets
#if FLEXT_SYS == FLEXT_SYS_PD
    add_anything(c,cb_px_anything); // for leftmost inlet
    px_class = class_new(gensym("Flext proxy"),NULL,NULL,sizeof(px_object),CLASS_PD|CLASS_NOINLET, A_NULL);
...
void Flext::jmax_proxy(fts_object_t *c, int winlet, fts_symbol_t s, int argc, const fts_atom_t *argv)
{
    Flext *o = thisObject(c);
