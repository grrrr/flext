...
// --- various symbols --------------------------------------------
	...
	*/
	...
	const t_symbol *MakeSymbol(const char *s);
...
// --- thread stuff -----------------------------------------------
	...
	*/
	inline bool ShouldExit() const { return shouldexit; }

	//! Check if current thread is the realtime system's thread
	inline bool IsSystemThread() const { pthread_t cur = pthread_self(); return pthread_equal(cur,thrid) != 0; }

	/*! \brief Yield to other threads
		\remark A call to this is only needed for systems with cooperative multitasking like MacOS<=9
	*/
	inline void ThrYield() { sched_yield(); }

	pthread_t thrid_t;

	/*! \brief Increase/Decrease priority of a thread
	*/
	bool ChangePriority(int dp,thrid_t thr = GetThreadId());
...
// --- thread stuff -----------------------------------------------
	...
	/*! \brief Check if current thread should terminate
		\todo Check for currently running thread
	*/
	static bool ShouldExit() const { return shouldexit; }

	//! Check if current thread is the realtime system's thread
	static bool IsSystemThread() const { pthread_t cur = pthread_self(); return pthread_equal(cur,thrid) != 0; }
