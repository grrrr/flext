...
void Flext::ItemCont::Resize(int nsz)
{
    if(nsz > memsize) {
...
void Flext::ItemCont::Add(Item *item,const t_symbol *tag,int inlet)
{
    FLEXT_ASSERT(tag);
...
bool Flext::ItemCont::Remove(Item *item,const t_symbol *tag,int inlet,bool free)
{
    FLEXT_ASSERT(tag);
...
Flext::Item *Flext::ItemCont::FindList(const t_symbol *tag,int inlet)
{
    FLEXT_ASSERT(tag);
...
typedef DataMap<Flext::t_classid,Flext::ItemCont *> ClassMap;

static ClassMap classarr[2];

Flext::ItemCont *Flext::GetClassArr(t_classid c,int ix) 
{
